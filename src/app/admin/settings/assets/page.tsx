...
export default function AssetSettingsPage() {
  ...
  const [reviewingRequest, setReviewingRequest] = useState<CustomTokenRequest | null>(null);
        ...
        <div className="flex space-x-8">
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              <span>
                커스텀 토큰 요청
                {customRequests.filter(req => req.status === 'pending').length > 0 && (
          ...
          {activeTab === 'requests' && (
            <div className="space-y-4">
              ...
              <div className="flex space-x-2">
                ...
                >
                  대기 중 ({customRequests.filter(req => req.status === 'pending').length})
                </button>
                <button
                ...
                >
                  승인됨 ({customRequests.filter(req => req.status === 'approved').length})
                </button>
                <button
                ...
                >
                  반려됨 ({customRequests.filter(req => req.status === 'rejected').length})
                </button>
              </div>
              ...
              <Card className="p-6">
                ...
                {(() => {
                  ...
                  return !Array.isArray(filteredRequests) || filteredRequests.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      {statusFilter === 'all' ? '요청 내역이 없습니다.' :
                       statusFilter === 'pending' ? '대기 중인 요청이 없습니다.' :
                       statusFilter === 'approved' ? '승인된 요청이 없습니다.' :
                       '반려된 요청이 없습니다.'}
                    </div>
                  ) : (
                    <div className="border rounded-lg">
                              ...
                              <TableCell>
                                <div>
                                  <div className="text-sm text-gray-900">
                                    {request.organizationName || request.userName || request.userId}
                                  </div>
                                  <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full mt-1 ${
                                    ...
                                    {request.memberType === 'corporate' ? '법인' : '개인'}
                                  </span>
                                </div>
                              </TableCell>
                              <TableCell>
