...
export default function FundSourceStep({
  initialData,
  onComplete,
  onBack,
}: FundSourceStepProps) {
  const [fundSource, setFundSource] = useState(initialData.fundSource || "");
  const [fundSourceDetail, setFundSourceDetail] = useState(
    initialData.fundSourceDetail || ""
  );
  const [transactionPurpose, setTransactionPurpose] = useState(
    initialData.transactionPurpose || ""
  );
  const [transactionPurposeDetail, setTransactionPurposeDetail] = useState(
    initialData.transactionPurposeDetail || ""
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (initialData.residentNumber) {
        ...
        if (birthPart.length === 6 && genderPart.length >= 1) {
          ...
          if (
            ...
            fullBirthDate === dateObj.toISOString().split("T")[0]
          ) {
      ...
      const newUser: any = {
        ...
        email:
          initialData.email ||
          `${initialData.phone?.replace(/-/g, "")}@temp.com`,
      ...
      if (initialData.residentNumber)
        newUser.residentNumber = initialData.residentNumber;
      ...
      if (initialData.accountNumber)
        newUser.accountNumber = initialData.accountNumber;
      if (initialData.accountHolder)
        newUser.accountHolder = initialData.accountHolder;
      if (initialData.financeCode)
        newUser.financeCode = initialData.financeCode;

      // 신분증 이미지 (있는 경우에만)
      if (initialData.idCardImageBase64)
        newUser.idCardImageBase64 = initialData.idCardImageBase64;

      // 셀피 이미지 (있는 경우에만)
      if (initialData.selfieImageBase64)
        newUser.selfieImageBase64 = initialData.selfieImageBase64;

      // eKYC 결과 타입 (있는 경우에만)
      if (initialData.kycResultType)
        newUser.kycResultType = initialData.kycResultType;

      // eKYC 원본 데이터 (있는 경우에만)
      if (initialData.kycReviewData)
        newUser.kycReviewData = initialData.kycReviewData;
      ...
      if (initialData.detailAddress)
        newUser.detailAddress = initialData.detailAddress;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
      ...
      <div className="space-y-4 mb-6">
        ...
        {transactionPurpose === "other" && (
          <div>
            ...
            <textarea
              value={transactionPurposeDetail}
              onChange={(e) => setTransactionPurposeDetail(e.target.value)}
              rows={4}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
              placeholder="거래 목적에 대해 상세히 설명해주세요"
            />
          </div>
        )}
      </div>
      ...
      <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
        <p className="text-sm text-gray-600">
          <strong className="text-gray-700">개인정보 처리 방침:</strong>{" "}
          입력하신 자금출처 정보는 금융거래법령에 따라 안전하게 보관되며,
      ...
      <div className="flex space-x-3 mt-6">
        ...
        <button
          ...
          disabled={
            !fundSource ||
            (fundSource === "other" && !fundSourceDetail.trim()) ||
            !transactionPurpose ||
            (transactionPurpose === "other" &&
              !transactionPurposeDetail.trim()) ||
            loading
          }
