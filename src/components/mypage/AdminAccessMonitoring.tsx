...
export default function AdminAccessMonitoring({
  isVisible,
  onClose,
}: AdminAccessMonitoringProps) {
  const [filterStatus, setFilterStatus] = useState<
    "all" | "success" | "failed" | "blocked"
  >("all");
  const [filterTimeRange, setFilterTimeRange] = useState<"24h" | "7d" | "30d">(
    "24h"
  );

  const [accessLogs, setAccessLogs] =
    useState<AccessLogEntry[]>(adminAccessLogs);
  ...
  const getLogCounts = () => {
    ...
    const blocked = filteredLogs.filter(
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        ...
        {filteredLogs.length > 0 ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400">필터 조건을 변경해 보세요.</p>
          </div>
        )}
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm">
              ...
              <div className="text-gray-700 space-y-1">
                ...
                <p>
                  • 정기적으로 접근 로그를 검토하고 이상 징후를 모니터링하세요
                </p>
              </div>
            </div>
          </div>
