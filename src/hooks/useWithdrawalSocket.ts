...
interface UseWithdrawalSocketOptions {
  userId: string | null;
  onWithdrawalUpdate?: (withdrawal: any) => void;
}

export function useWithdrawalSocket({ userId, onWithdrawalUpdate }: UseWithdrawalSocketOptions) {
  ...
  useEffect(() => {
    ...
    socket.on('authenticated', (response: { success: boolean; message: string; roomName?: string }) => {
      if (response.success) {
        console.log('WebSocket 인증 성공:', response.roomName);
      } else {
        console.error('WebSocket 인증 실패:', response.message);
